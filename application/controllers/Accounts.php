...
<input type=\"hidden\" name=\"HeadName\" id=\"HeadName\" class=\"form_input\" value=\"" . $role_reult->HeadName . "\"/>
    ...
    public function debit_voucher()
    {
        ...
        $data['transaction_parent'] = $this->accounts_model->Transaction_Parent();
    ...
    {
        $query = $this->db->select('a.*, b.supplier_price')
            ->from('product_information a')
            ->join('supplier_product b', 'b.product_id = a.product_id', 'left')
    ...
    public function save_opening_inventory()
    {
        ...
        $all_unit_price = $this->input->post('unit_price', TRUE);
        $all_total_price = $this->input->post('total_price', TRUE);
        ...
        for ($i = 0; $i < count($all_pr); $i++) {
            ...
            if ($db_products->num_rows() > 0) {
                ...
                $data = array(
                    ...
                    'unit_price'   => $all_unit_price[$i],
                    'total_price'   => $all_total_price[$i] + (int)$res[0]['total_price'],
                );
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////// Accounts Module Newly Developed @ANTAR NANDI /////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public function debit_voucher_new()
    {
        $CI = &get_instance();
        $CI->load->model('Web_settings');
        $CI->load->model('Settings');
        ...
        $content = $this->parser->parse('newaccount/debit_voucher_new', $data, true);
        $this->template->full_admin_html_view($content);
    }
    public function get_childhead($parent_head)
    {
        $html = '<option>' . "Select" . '</option>';

        if (!empty($parent_head)) {
            $result =  $this->accounts_model->get_child_by_parent($parent_head);
            if (!empty($result)) {
                foreach ($result as $key => $value) {
                    $html .= '<option value=' . $value->HeadCode . '>' . $value->HeadCode . " - " . $value->HeadName . '</option>';
                    $child = $this->accounts_model->get_child_by_parent($value->HeadName);
                    if ($child) {
    ...
    public function create_debit_voucher_new()
    {
        $this->form_validation->set_rules('cmbDebit', display('cmbDebit'), 'max_length[100]');

        $_SESSION['pay_to'] = $this->input->post('pay_to', TRUE);
        if ($this->form_validation->run()) {
            if ($this->accounts_model->insert_debitvoucher_new()) {
                $this->session->set_flashdata('message', display('save_successfully'));
                redirect("accounts/print_voucher/" . addslashes(trim($this->input->post('txtVNo', TRUE))) . "/");
            } else {
                $this->session->set_flashdata('error_message',  display('please_try_again'));

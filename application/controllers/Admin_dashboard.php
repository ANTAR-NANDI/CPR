...
// exit();
    ...
    public function retrieve_dateWise_PurchaseReports()
    {
        ...
        if ($outlet_id === 'All') {
            ...
            $outlet_id = $this->input->get('outlet_id', TRUE);
        }
        ...
        if (!empty($alldata)) {
            $perpagdata = $this->Reports->count_retrieve_dateWise_PurchaseReports($start_date, $end_date, $outlet_id, $supplier_id);
        } else {
            $perpagdata = 25;
        }
        $config["base_url"] = base_url('Admin_dashboard/retrieve_dateWise_PurchaseReports/');
        $config["total_rows"] = $this->Reports->count_retrieve_dateWise_PurchaseReports($start_date, $end_date, $outlet_id, $supplier_id);
        ...
        $links = $this->pagination->create_links();
        #
        $content = $CI->lreport->retrieve_dateWise_PurchaseReports($start_date, $end_date, $links, $config["per_page"], $page, $outlet_id, $supplier_id);
        $this->template->full_admin_html_view($content);
    }
    ...
    public function sale_report_paytype_wise()
    {
        ...
        $day_total_cheque = array_sum(array_column($sales_data, 'pay_type_cheque'));
        ...
        $data = array(
            ...
            'day_total_cheque'     => $day_total_cheque,
    ...
    public function supplier_wise_sales_report()
    {
        ...
        $content = $CI->lreport->supplier_wise_sales_report();

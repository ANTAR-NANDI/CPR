...
class Ccustomer extends CI_Controller
{
    ...
    public function insert_customer()
    {
        ...
        $outlet_id = $this->session->userdata('outlet_id') ? $this->session->userdata('outlet_id') : $this->session->userdata('user_id');
        // echo "<pre>";
    ...
    // Customer Payment History By ANTAR NANDI
    public function customer_due_payment($customer_id)
    {
        ...
        $data['customer_payment'] = $this->Customers->customer_due_payment($customer_id);
        $content = $this->parser->parse('customer/customer_payment', $data, true);
        $this->template->full_admin_html_view($content);
    }
    public function customer_due_update()
    {
        $CI = &get_instance();
        $this->auth->check_admin_auth();
        $CI->load->model('Customers');
        $customer_id = $this->input->post('customer_id', TRUE);
        $status = $CI->Customers->customer_due_update_entry();
        if($status)
        {
            $this->session->set_userdata(array('message' => 'Customer Payment successfully Updated'));
            redirect(base_url('Ccustomer/customer_due_payment/'.$customer_id));
        }
        else{

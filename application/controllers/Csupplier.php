...
class Csupplier extends CI_Controller
{
    ...
    // Supplier Payment History By ANTAR NANDI
    public function supplier_due_payment($supplier_id)
    {
        $CI = &get_instance();
        $this->auth->check_admin_auth();
        $CI->load->model('Suppliers');
        $data['title'] = 'Supplier Due Payment';
        $data['supplier_id'] = $supplier_id;
        $data['supplier_payment'] = $this->Suppliers->supplier_due_payment($supplier_id);
        $content = $this->parser->parse('supplier/supplier_payment', $data, true);
        $this->template->full_admin_html_view($content);
    }
    public function supplier_due_update()
    {
        $CI = &get_instance();
        $this->auth->check_admin_auth();
        $CI->load->model('Suppliers');
        $supplier_id = $this->input->post('supplier_id', TRUE);
        $status = $CI->Suppliers->supplier_due_update_entry();
        if($status)
        {
            $this->session->set_userdata(array('message' => 'Supplier Payment successfully Updated'));
            redirect(base_url('Csupplier/supplier_due_payment/'.$supplier_id));
        }
        else{

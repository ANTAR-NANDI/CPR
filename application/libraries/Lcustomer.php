...
class Lcustomer
{
    ...
    public function customer_list()
    {
        ...
        $data['outlet_user']       = $CI->Warehouse->get_outlet($outlet_id);
    ...
    public function customer_ledger_cheque($customer_id, $start, $end)
    {
        $CI = &get_instance();
        $CI->load->model('Customers');
        $CI->load->model('Web_settings');
        ...
        $customer = $CI->Customers->customer_list_ledger();
        $customer_detail = $CI->Customers->customer_personal_data($customer_id);
        $ledger   = $CI->Customers->customerledger_searchdata_cheque($customer_id, $start, $end);
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        ...
        $singlecustomerdetails = $CI->parser->parse('customer/customer_ledger_report_cheque', $data, true);
        return $singlecustomerdetails;
    }
    ...
    public function customer_ledger_report_cheque($links, $per_page, $page)
    {
        $CI = &get_instance();
        $CI->load->model('Customers');
        $CI->load->model('Web_settings');
        ...
        $ledger   = $CI->Customers->customer_product_buy_cheque($per_page, $page);
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
    ...
    public function customer_edit_data($customer_id)
    {
        ...
        $data = array(
            ...
            'card_number'     => $customer_detail[0]['card_number'],
            'membership_id'     => $customer_detail[0]['membership_id'],
            'friend_card'     => $customer_detail[0]['friend_card'],
            'customer_id_two'     => $customer_detail[0]['customer_id_two'],
            'customer_name'   => $customer_detail[0]['customer_name'],
            'customer_name_bn'   => $customer_detail[0]['customer_name_bn'],
            ...
            'outlet_id'          => $customer_detail[0]['outlet_id'],

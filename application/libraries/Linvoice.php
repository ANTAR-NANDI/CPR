...
class Linvoice
{
    ...
    //Pos invoice add form new

    public function pos_invoice_add_form_new()
    {
        $CI = &get_instance();
        $CI->load->model('Invoices');
        $CI->load->model('Web_settings');
        ...
        // echo '<pre>';print_r($data);exit();
        $invoiceForm = $CI->parser->parse('invoice/add_pos_invoice_form_new', $data, true);
        return $invoiceForm;
    }
    ...
    public function invoice_html_data_manual($invoice_id, $manage = null)
    {
        ...
        $reference_invoice = '';
        $return_invoice = '';
        if ($invoice_detail[0]['invoice_type_status'] == 2) {
            $reference_invoice = $CI->db->select('invoice')->from('invoice')->where('return_replace_invoice_id', $invoice_id)->where('is_return_replace', 1)->get()->row()->invoice;
        }

        if ($invoice_detail[0]['invoice_type_status'] == 1 && $invoice_detail[0]['is_return_replace'] == 1) {
            $return_invoice = $CI->db->select('invoice')->from('invoice')->where('invoice_id', $invoice_detail[0]['return_replace_invoice_id'])->get()->row()->invoice;
        }

        // echo '<pre>';
        // print_r($invoice_detail);
        // print_r($reference_invoice);
        // print_r($return_invoice);
        // exit();
        ...
        $data = array(
            ...
            'invoice_type_status'    => $invoice_detail[0]['invoice_type_status'],
            'is_return_replace'    => $invoice_detail[0]['is_return_replace'],
            'return_replace_invoice_id'    => $invoice_detail[0]['return_replace_invoice_id'],
    ...
    public function getSettingData()
    {
        ...
        $data = [
            'title' => display('invoice_setting'),
            // 'pos_setting_list' => $pos_setting_list,
            'SettingData' => $CI->Invoices->getSettingData(),
            'language'          => $language,
        ];

        // $data['SettingData'] = $CI->Invoices->getSettingData();
        // echo "<pre>" ;print_r($data);exit();

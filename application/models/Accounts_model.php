...
class Accounts_model extends CI_Model
{
    ...
    public function Transacc()
    {
        return  $data = $this->db->select("*")
            ->from('acc_coa')
            ->like('HeadCode', 40, 'after')
            ->not_like('HeadCode', 40, 'none')
    ...
    public function cr_transacc()
    {
        return  $data = $this->db->select("*")
            ->from('acc_coa')
            ->like('HeadCode', 30, 'after')
            ->not_like('HeadCode', 30, 'none')
         //   ->where('IsTransaction', 1)
            ->where('IsActive', 1)
            ->order_by('HeadName')
            ->get()
    ...
    public function profit_loss_serach($outlet_id = null,$from_date,$to_date)
    {
        ...
        if ($outlet_id != 1) {
            if ($outlet_id == 'HK7TGDT69VFMXB7') {
                $product_sale = $this->db->select('(sum(credit)-sum(debit)) as product_sale')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('COAID', 303)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
                $indirect_income = $this->db->select('(sum(credit)-sum(debit)) as indirect_income')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->like('COAID', '3040')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
                $service_income = $this->db->select('(sum(credit)-sum(debit)) as service_income')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('COAID', 304)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
                $sale_return = $this->db->select('(sum(credit)-sum(debit)) as sale_return')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('COAID', 301)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();

                $indirect_income_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.id IS NULL')->where('HeadType', 'I')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '3040')->group_by('a.HeadCode')->get()->result();
                ...
                $product_purchase = $this->db->select('(sum(debit)-sum(credit)) as product_purchase')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('COAID', 402)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
                $opening_inventory = $this->db->select('(sum(debit)-sum(credit)) as opening_inventory')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('COAID', 10205)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
                $direct_expense = $this->db->select('(sum(debit)-sum(credit)) as direct_expense')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4010')->get()->result_array();
                $indirect_expense = $this->db->select('(sum(debit)-sum(credit)) as indirect_expense')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.id IS NULL')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4040')->get()->result_array();

                $expense = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.id IS NULL')->where('HeadType', 'E')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '4010')->group_by('a.HeadCode')->get()->result();
                $indirect_expense_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.id IS NULL')->where('HeadType', 'E')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '4040')->group_by('a.HeadCode')->get()->result();
            } else {

                $product_sale = $this->db->select('(sum(credit)-sum(debit)) as product_sale')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 303)->get()->result_array();
                $indirect_income = $this->db->select('(sum(credit)-sum(debit)) as indirect_income')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '3040')->get()->result_array();
                $service_income = $this->db->select('(sum(credit)-sum(debit)) as service_income')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 304)->get()->result_array();
                $sale_return = $this->db->select('(sum(credit)-sum(debit)) as sale_return')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 301)->get()->result_array();

                $indirect_income_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.outlet_id', $outlet_id)->where('HeadType', 'I')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '3040')->group_by('a.HeadCode')->get()->result();

                $purchase_discount = $this->db->select('(sum(credit)-sum(debit)) as purchase_discount')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 301)->get()->result_array();
                $sales_discount = $this->db->select('(sum(debit)-sum(credit)) as sales_discount')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 406)->get()->result_array();

                $purchase_return = $this->db->select('(sum(debit)-sum(credit)) as purchase_return')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 307)->get()->result_array();
                $production_expense = $this->db->select('(sum(debit)-sum(credit)) as production_expense')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 409)->get()->result_array();
                $product_purchase = $this->db->select('(sum(debit)-sum(credit)) as product_purchase')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 402)->get()->result_array();
                $opening_inventory = $this->db->select('(sum(debit)-sum(credit)) as opening_inventory')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->where('COAID', 10205)->get()->result_array();
                $direct_expense = $this->db->select('(sum(debit)-sum(credit)) as direct_expense')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4010')->get()->result_array();
                $indirect_expense = $this->db->select('(sum(debit)-sum(credit)) as indirect_expense')->from('acc_transaction')->join('outlet_warehouse', 'acc_transaction.CreateBy = outlet_warehouse.user_id', 'left')->where('outlet_warehouse.outlet_id', $outlet_id)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4040')->get()->result_array();

                $expense = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.outlet_id', $outlet_id)->where('HeadType', 'E')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '4010')->group_by('a.HeadCode')->get()->result();
                $indirect_expense_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->join('outlet_warehouse x', 'b.CreateBy = x.user_id', 'left')->where('x.outlet_id', $outlet_id)->where('HeadType', 'E')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '4040')->group_by('a.HeadCode')->get()->result();
            }
        } else {
            $product_sale = $this->db->select('(sum(credit)-sum(debit)) as product_sale')->from('acc_transaction')->where('COAID', 303)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
            $indirect_income = $this->db->select('(sum(credit)-sum(debit)) as indirect_income')->from('acc_transaction')->like('COAID', '3040')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
            $service_income = $this->db->select('(sum(credit)-sum(debit)) as service_income')->from('acc_transaction')->where('COAID', 304)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
            $sale_return = $this->db->select('(sum(credit)-sum(debit)) as sale_return')->from('acc_transaction')->where('COAID', 301)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();

            $indirect_income_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->where('HeadType', 'I')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '3040')->group_by('a.HeadCode')->get()->result();
            ...
            $product_purchase = $this->db->select('(sum(debit)-sum(credit)) as product_purchase')->from('acc_transaction')->where('COAID', 402)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
            $opening_inventory = $this->db->select('(sum(debit)-sum(credit)) as opening_inventory')->from('acc_transaction')->where('COAID', 10205)->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->get()->result_array();
            $direct_expense = $this->db->select('(sum(debit)-sum(credit)) as direct_expense')->from('acc_transaction')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4010')->get()->result_array();
            $indirect_expense = $this->db->select('(sum(debit)-sum(credit)) as indirect_expense')->from('acc_transaction')->where('acc_transaction.VDate >=',$from_date)->where('acc_transaction.VDate <=',$to_date)->like('COAID', '4040')->get()->result_array();

            $expense = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->where('HeadType', 'E')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->like('COAID', '4010')->group_by('a.HeadCode')->get()->result();
            $indirect_expense_c = $this->db->select('*')->from('acc_coa a')->join('acc_transaction b', 'a.HeadCode=b.COAID')->where('b.VDate >=',$from_date)->where('b.VDate <=',$to_date)->where('HeadType', 'E')->like('COAID', '4040')->group_by('a.HeadCode')->get()->result();
        }
    ...
    public function get_child_by_parent($parent_head)
    {
        $this->db->select('acc_coa.*')
        ->where('IsActive', 1)
        ->where('PHeadName', $parent_head)
        ->where('IsTransaction', 1);
    ...
    public function insert_debitvoucher()
    {
        $CI = &get_instance();
        $CI->load->model('Settings');
        $voucher_date = $this->input->post('date', TRUE);
        $voucher_no = addslashes(trim($this->input->post('txtVNo', TRUE)));
        $voucher_type = "DV";
        $pay_type = $this->input->post('paytype', TRUE);
        ...
        $remarks =  addslashes(trim($this->input->post('remarks', TRUE)));
        $parent_head = $this->input->post('parent_head', TRUE);
        $headcode = $this->input->post('headcode', TRUE);
        $amount = $this->input->post('amount', TRUE);
        $child_head = $this->input->post('child_head', TRUE);
        $IsPosted = 1;
        $IsAppove = 0;
        $CreateBy = $this->session->userdata('user_id');
        $createdate = date('Y-m-d H:i:s');

        for ($i = 0; $i < count($child_head); $i++) {
            $coaid = $child_head[$i];
            $value = $amount[$i];
            // Cash Section
            if ($this->input->post('paytype', TRUE) == 1) {
                $headinfo = $this->db->select('*')->from('acc_coa')->where('HeadCode', $coaid)->get()->row()->HeadName;
                ...
                $this->db->insert('acc_transaction', $acc_dedit);
                $cash_credit = array(
                    'VNo'            =>  $voucher_no,
                    'Vtype'          =>  $voucher_type,
                    'VDate'          =>  $voucher_date,
                    'COAID'          =>  1020101,
                    'Narration'      =>  "Cash Received for Voucher",
                    'Credit'         =>  $value,
                    'Debit'          =>  0,
                    'IsPosted'       =>  1,
                    'CreateBy'       =>  $CreateBy,
               ...
               $this->db->insert('acc_transaction', $cash_credit);
            }
            // Bank Section
            if ($this->input->post('paytype', TRUE) == 4) {
                if (!empty($bank_id)) {
                  ...
                  $this->db->insert('acc_transaction', $account_debit);
                    $bank_credit = array(
                        'VNo'            =>  $voucher_no,
                        'Vtype'          =>  $voucher_type,
                        'VDate'          =>  $voucher_date,
                        'COAID'          =>  $bankcoaid,
                        'Narration'      =>  "Cheque Registered",
                        'Credit'         =>  $value,
                        'Debit'          =>  0,
                        'IsPosted'       =>  1,
                        'CreateBy'       =>  $CreateBy,
                  ...
                  $this->db->insert('cheque_receipt', $cheque_data);
                } 
            }
            // Bkash Section
            if ($this->input->post('paytype', TRUE) == 3)
                {
                    if (!empty($bkash_id)) {
                        ...
                        $this->db->insert('acc_transaction', $acc_debit);
                        $bkash_credit = array(
                            'VNo'            =>  $voucher_no,
                            'Vtype'          =>  $voucher_type,
                            'VDate'          =>  $voucher_date,
                            'COAID'          =>  $bkashcoaid,
                            'Narration'      =>  "Bkash Received for Voucher",
                            'Credit'         =>  $value,
                            'Debit'          =>  0,
                            'IsPosted'       =>  1,
                            'CreateBy'       =>  $CreateBy,
                    ...
                    $this->db->insert('acc_transaction', $bkash_credit);
                    }    
                }
              // Nagad Payment
            if ($this->input->post('paytype', TRUE) == 5) 
            {
                ...
                $bkash_credit = array(
                    ...
                    'VDate'          =>  $voucher_date,
                    'COAID'          =>  $nagadcoaid,
                    'Narration'      =>  "Nagad Received for Voucher",
                    'Credit'         =>  $value,
                    'Debit'          =>  0,
                    'IsPosted'       =>  1,
                    'CreateBy'       =>  $CreateBy,
              ...
              if ($this->input->post('paytype', TRUE) == 6) 
              {
                ...
                if (!empty($card_id)) {
                  ...
                  $bkash_credit = array(
                      ...
                      'VDate'          =>  $voucher_date,
                      'COAID'          =>  $card_bankcoaid,
                      'Narration'      =>  "Card Received for Voucher",
                      'Credit'         =>  $value,
                      'Debit'          =>  0,
                      'IsPosted'       =>  1,
                      'CreateBy'       =>  $CreateBy,

...
class Customers extends CI_Model
{
    ...
    public function getCustomerList($postData = null)
    {
        ...
        foreach ($records as $record) {
            ...
            $customer_details = '  <a href="' . $base_url . 'Ccustomer/customer_due_payment/' . $record->customer_id . '" class="" >' . $record->customer_name .  '</a>';
    ...
    public function customer_product_buy($per_page, $page)
    {
        ...
        $this->db->select('a.*,b.HeadName,i.invoice_number');
        $this->db->from('acc_transaction a');
        $this->db->join('acc_coa b', 'a.COAID=b.HeadCode');
        $this->db->join('service_invoice i', 'i.service_invoice_id=a.VNo');
    ...
    public function opening_balance($customer_id)
    {
        $date = date('Y-m-d');
        $this->db->select("((select ifnull(sum(Debit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)-(select ifnull(sum(Credit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)) as opening_balance");
        $this->db->from('customer_information a');
        $this->db->join('acc_coa b', 'a.customer_id = b.customer_id', 'left');
        $this->db->join('acc_transaction', 'b.HeadCode = acc_transaction.COAID', 'left');
        $this->db->where('b.customer_id',$customer_id);
        $this->db->where('acc_transaction.VDate <=',$date);
        return $this->db->get()->result();
        //
;    }
public function closing_balance($customer_id)
    {
        $this->db->select("((select ifnull(sum(Debit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)-(select ifnull(sum(Credit),0) from acc_transaction where COAID= `b`.`HeadCode` AND IsAppove = 1)) as closing_balance");
        $this->db->from('customer_information a');
        $this->db->join('acc_coa b', 'a.customer_id = b.customer_id', 'left');
        $this->db->where('b.customer_id',$customer_id);
        return $this->db->get()->result();
;    }
 // Customer due Payment View
 public function customer_due_payment($customer_id)
 {
     $this->db->select('invoice.*');
     $this->db->from('invoice');
     $this->db->where('invoice.due_amount >', 0);
     $this->db->where('invoice.customer_id', $customer_id);
     return $this->db->get()->result_array();
         
 }

 // Customer due Payment Entry
 public function customer_due_update_entry(){
     ...
     if($total_paid_amount != $total_pay_amount)
     {
         ...
         // Get The Supplier Headcode
         $cusinfo = $this->db->select('*')->from('customer_information')->where('customer_id', $customer_id)->get()->row();
         $cus_head = $cusinfo->customer_id . '-' . $cusinfo->customer_name;
         $cus_coa = $this->db->select('*')->from('acc_coa')->where('HeadName', $cus_head)->get()->row();
         $customer_headcode = $cus_coa->HeadCode;

         // Get the Payment Headcode
             ...
             if($value > 0)
             {
               ///////////////////////////////////////////////////////////////////////////
                    $cosdr = array(
                        ...
                        'VDate' => $createdate,
                        'COAID' => $customer_headcode,
                        'Narration' => 'Customer debit For Invoice',
                        'Debit' => 0,
                        'Credit' => $value,
                        'IsPosted' => 1,
                        'CreateBy' => $createby,
              ...
              ///////////////////////////////////////////////////////////////////////////////////////////
               $payment_credit = array(
                ...
                $this->db->insert('acc_transaction', $payment_credit);

             ////////////////////////////////////////////////////////////////////////////////////
             $data = array(
                'due_amount' => $total_amount[$key] - ($value + (float)$previous_paid_amount[$key]),
                'paid_amount' => $value + $previous_paid_amount[$key],
              );
    
    
             $this->db->where('invoice_id', $invoice_id[$key]);

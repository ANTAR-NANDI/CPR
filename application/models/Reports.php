...
class reports extends CI_Model
{
    ...
    public function getCheckListNew2($postData = null, $post_product_id = null, $product_sku = null, $pr_status = null, $from_date = null, $to_date = null, $value = null)
    {
        ...
        foreach ($records as $record) {
            ...
            $stockout_for_purchase_price = $this->db->select('sum(product_purchase_details.qty) as totalPurchaseQnty, sum(product_purchase_details.total_amount) as purchaseprice')
                ...
                ->get()
                ->row();
            $opening_inventory_price = $this->db->select('stock_qty,total_price')->from('opening_inventory')->where('product_id',$record->product_id)->get()->row();
            $stockout_for_weighted_average = 0;

            if ($stockout_for_purchase_price->totalPurchaseQnty == 0) {
                $stockout_for_weighted_average = $stockout_for_purchase_price->purchaseprice  / 1;
            } else {
                $stockout_for_weighted_average = ($stockout_for_purchase_price->purchaseprice + $opening_inventory_price->total_price) / ($stockout_for_purchase_price->totalPurchaseQnty + $opening_inventory_price->stock_qty);
            }
    ...
    public function todays_sales_report(
        ...
        $today = date('Y-m-d');
        $this->db->select('product_brand.brand_name,a.date,a.invoice,a.invoice_id,a.due_amount,a.outlet_id,a.changeamount,
        ...
        $this->db->join('product_brand', 'product_brand.id = pi.brand_id', 'left');
        $this->db->where('a.date', $today);
        if ($outlet_id) {
            $this->db->where('a.outlet_id', $outlet_id);
        }
        ...
        foreach ($query as $key => $value) {
            ...
            $final_array[$value['invoice_id']]['invoice_id'] = $value['invoice_id'];
            $final_array[$value['invoice_id']]['invoice'] = $value['invoice'];
    ...
    public function total_service_amount($date = null)
    {
        ...
        $this->db->where('invoice_date >=', $days['start_date']);
        $this->db->where('invoice_date <=', $days['end_date']);
        $query = $this->db->get();

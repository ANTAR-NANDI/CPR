...
class Rqsn extends CI_Model
{
    ...
    public function get_outlet_by_id($outlet_id)
    {
        $outlet = $this->db->select('*')
            ->from('outlet_warehouse')
            ->where('outlet_warehouse.outlet_id', $outlet_id)
            ->get()
            ->row();
        return $outlet;
    }
    ...
    public function approve_rqsn()
    {
        ...
        if ($outlet_id) {
            $this->db->where('a.to_id', $outlet_id);
        }
        ...
        foreach ($records as $record) {
            ...
            $expired_stock = $this->Reports->getExpiryCheckList($record->product_id, 1)['expired_stock'];

            if ($record->from_id == 'HK7TGDT69VFMXB7') {
                $outlet_name = "Central Warehouse";
            } else {
                $outlet_name = $this->get_outlet_by_id($record->from_id)->outlet_name;
            }
            ...
            $data[] = array(
                ...
                'product_id' => $record->product_id,
                'stok_quantity' => sprintf('%0.2f', $expired_stock),
    ...
    public function approve_outlet()
    {
        ...
        // $outlet_list = $CI->Warehouse->outlet_or_cw_logged_in();
        // $own_outlet_id = $outlet_list[0]['outlet_id'];
        ...
        $this->db->select('*,SUM(a_qty) as a_qty');
        $this->db->from('rqsn a');
        $this->db->join('rqsn_details b', 'a.rqsn_id=b.rqsn_id');
        $this->db->join('product_information d', 'd.product_id=b.product_id');
        $this->db->where('b.status', 2);
        $this->db->where('b.is_return', 0);
        if ($outlet_id) {
            $this->db->where('a.from_id', $outlet_id);
        }
        $this->db->group_by(array('b.product_id', 'b.rqsn_id'));
        $approveinfo = $this->db->get()->result();

        // echo '<pre>';
        // print_r($approveinfo);
        // // echo "outlet_id = " . $outlet_id . " user_id = " . $user_id;
        // exit();
        ...
        foreach ($approveinfo as $record) {
            ...
            if ($record->from_id == 'HK7TGDT69VFMXB7') {
                $from->outlet_name = 'Central Warehouse';
            }
    ...
    public function return_rcv()
    {
        ...
        $this->db->from('rqsn a');
        $this->db->join('rqsn_details b', 'a.rqsn_id=b.rqsn_id');
        $this->db->join('outlet_warehouse c', 'c.outlet_id=a.from_id', 'left');
        $this->db->join('central_warehouse e', 'e.warehouse_id=a.to_id', 'left');
        $this->db->join('product_information d', 'd.product_id=b.product_id');
    ...
    public function approved($data = [])
    {
        ...
        if ($det[0]['from_id'] == 'HK7TGDT69VFMXB7') {
            $det[0]['outlet_id'] = 'HK7TGDT69VFMXB7';
            $det[0]['warehouse_id'] = '';
            $det[0]['user_id'] = '6TaboNUiB9PeiIu';
            $det[0]['customer_id'] = 'Central Warehouse';
            $det[0]['outlet_name'] = 'Central Warehouse';
            $det[0]['address'] = '';
        } else {
            $outlet = $this->get_outlet_by_id($det[0]['from_id']);

            $det[0]['outlet_id'] = $outlet->outlet_id;
            $det[0]['warehouse_id'] = $outlet->warehouse_id;
            $det[0]['user_id'] = $outlet->user_id;
            $det[0]['customer_id'] = $outlet->customer_id;
            $det[0]['outlet_name'] = $outlet->outlet_name;
            $det[0]['address'] = $outlet->address;
        }
        ...
        $this->db->where('rqsn_detail_id', $data['rqsn_detail_id']);
        $this->db->delete('rqsn_details');


        $stock_details = $this->Reports->getExpiryCheckList($data['product_id'], 1)['aaData'];

        $stock = $stock_details[0]['stok_quantity'];

        $rest = $data['a_qty'] - $stock;


        $array = array();

        $aprv_qty = $rest > 0 ? $stock : $data['a_qty'];

        $first_array = array(
            'rqsn_detail_id' => date('Ymdhs'),
            'rqsn_id' => $det[0]['rqsn_id'],
            'product_id' => $data['product_id'],
            'purchase_id' => $stock_details[0]['purchase_id'],
            'quantity' => $det[0]['quantity'],
            'unit' => $det[0]['unit'],
            'a_qty' => $aprv_qty,
            'status' => $data['status'],
            'isaprv' => 1,
        ...
        foreach ($array as $ar) {
            ...
            if ($ar['a_qty'] > 0) {
                $result = $this->db->insert('rqsn_details', $ar);
            }
        }
    ...
    public function delete_rqsn($rqsn_id, $product_id)
    {
        // echo "<pre>";
        // print_r($rqsn_id);
        // exit();
    ...
    public function delete_rqsn_center($rqsn_id, $product_id)
    {
        // echo "<pre>";
        // print_r($rqsn_id);
        // exit();
        $this->db->where('rqsn_id', $rqsn_id)->where('product_id', $product_id)
            ->delete('rqsn_details');
        if ($this->db->affected_rows()) {
            return true;
        } else {
    ...
    public function outlet_stock($postData = null, $post_product_id = null, $from_date = null, $to_date = null)
    {
        ...
        // echo "<pre>";
        // echo $from_date . " " . $to_date . " " . $op_date;
        // exit();
        ...
        foreach ($records as $record) {
            ...
            $opening_stockout = $this->db->select('sum(qty) as totalPurchaseQnty,sum(damaged_qty) as damaged_qty,Avg(rate) as purchaseprice')
                ->from('product_purchase_details')
                ->join('product_purchase', 'product_purchase.purchase_id = product_purchase_details.purchase_id')
                ->where('product_purchase.outlet_id', $outlet_id)
            ...
            // echo "<pre>";
            // print_r($stockout->totalPurchaseQnty);
            // exit();
            ...
            $total_in = $stockout->totalPurchaseQnty + $total_purchase->total_purchase - $return_given->totalReturnQnty;

            $total_return = $return_given->totalReturnQnty + $product_return->totalReturn;

            // echo "<pre>";
            // echo $total_in . " " .
            //     $total_purchase->total_purchase . " " . $stockout->totalPurchaseQnty;
            // // echo $outlet_id;
            // exit();
            ...
            if ($to_date)
                $this->db->where('c.date <=', $to_date);
            $this->db->select('a.*,c.*,,SUM(a.a_qty) as total_transfer');
            $this->db->from('rqsn_details a');
            $this->db->join('rqsn c', 'a.rqsn_id=c.rqsn_id');
            // $this->db->join('outlet_warehouse d', 'c.from_id=d.outlet_id');
            $this->db->where('a.product_id', $record->product_id);
            $this->db->where('c.to_id', $outlet_id);
            $this->db->where('a.isaprv', 1);
            $this->db->where('a.isrcv', 1);
            $this->db->where('a.is_return', 0);
            $total_transfer = $this->db->get()->row();
            ...
            $out_qty = (!empty($total_sale->total_sale) ? $total_sale->total_sale : 0) + (!empty($total_transfer->total_transfer) ? $total_transfer->total_transfer : 0);
            ...
            $opening_stock_new =  ((!empty($total_purchase->total_purchase) ? $total_purchase->total_purchase : 0) + (!empty($opening_stockout->totalPurchaseQnty) ? $opening_stockout->totalPurchaseQnty : 0) - $out_qty - $return_product) + $diff;
            ...
            //     $this->db->where('a.isrcv', 1);
            //     $this->db->where('a.is_return', 0);
            //     $opening_total_transfer = $this->db->get()->row();
            ...
            //     $opening_out_qty = (!empty($opening_total_sale->total_sale) ? $opening_total_sale->total_sale : 0) + (!empty($opening_total_transfer->total_transfer) ? $opening_total_transfer->total_transfer : 0);
            ...
            //     $opening_stock =  ((!empty($opening_total_purchase->total_purchase) ? $opening_total_purchase->total_purchase : 0) + (!empty($stockout->totalPurchaseQnty) ? $stockout->totalPurchaseQnty : 0) - $opening_out_qty - $return_product) + $diff;
            // } else {
            //     $opening_stock = $opening_stock_new;
            // }
            ...
            if ($from_date) {
                $opening_stock = $this->stock(null, $new_opening_from_date, $outlet_id, $record->product_id, $record->finished_raw);
            } else {
                $opening_stock = $this->stock($new_opening_from_date, $new_opening_to_date, $outlet_id, $record->product_id, $record->finished_raw);
            }

            if ($to_date) {
                $closing_stock = $this->stock(null, $to_date, $outlet_id, $record->product_id, $record->finished_raw);
            } else {
                $closing_stock = $this->stock($from_date, $to_date, $outlet_id, $record->product_id, $record->finished_raw);
            }
            ...
            //     $this->db->where('a.isrcv', 1);
            //     $this->db->where('a.is_return', 0);
            //     $closing_total_transfer = $this->db->get()->row();
            ...
            //     $closing_out_qty = (!empty($closing_total_sale->total_sale) ? $closing_total_sale->total_sale : 0) + (!empty($closing_total_transfer->total_transfer) ? $closing_total_transfer->total_transfer : 0);

            //     $diff = (!empty($phy_count->phy_qty) ? $phy_count->phy_qty : 0);

            //     $return_product = (!empty($return_given->totalReturnQnty) ? $return_given->totalReturnQnty : 0);

            //     $closing_stock =  ((!empty($closing_total_purchase->total_purchase) ? $closing_total_purchase->total_purchase : 0) + (!empty($stockout->totalPurchaseQnty) ? $stockout->totalPurchaseQnty : 0) - $closing_out_qty - $return_product) + $diff;
            // } else {
            //     $closing_stock = $stock;
            // }
            ...
            $data[] = array(
                ...
                'damagedQnty'   => sprintf('%0.2f', ($stockout->damaged_qty ? $stockout->damaged_qty : 0)),
                'totalSalesQnty' => sprintf('%0.2f', $out_qty),
                'dispatch' => $total_sale->total_sale,
                'return_given' => sprintf('%0.2f', ($total_return)),
    ...
    public function outlet_stock2($postData = null, $post_product_id = null, $from_date = null, $to_date = null)
    {
        // echo "<pre>";
        // print_r($this->input->post('value', TRUE));
        // exit();
        ...
        foreach ($records as $record) {
            ...
            //             'category'  => (!empty($record->name) ? $record->name : ''),
            //             'totalPurchaseQnty' => sprintf('%0.2f', $total_in),
            //             'damagedQnty'   => sprintf('%0.2f', ($total_damage)),
            ...
            // }
            // if ($value == 3) {
            //     if ($closing_stock > 0 || $total_damage > 0 || $total_return_given > 0 || $total_out > 0 || $total_in > 0) {
            //         $data[] = array(
            //             'sl'            =>   $sl,
            //             'product_name'  =>  $record->product_name_bn,
            //             'product_model' =>  $record->product_model,
            ...
            //     if (($closing_stock > 0 || $total_damage > 0 || $total_return_given > 0 || $total_out > 0 || $total_in > 0) && $closing_stock > 0) {
            ...
            //     if (($closing_stock < 1 && $total_damage < 1 && $total_return_given < 1 && $total_out < 1 && $total_in < 1)) {
            //         $data[] = array(
            //             'sl'            =>   $sl,
            //             'product_name'  =>  $record->product_name_bn,
            //             'product_model' =>  $record->product_model,
    ...
    public function availbale_outlet_stock2($postData = null, $post_product_id = null, $from_date = null, $to_date = null, $outlet_id = null)
    {
        // echo "<pre>";
        // echo "1";
        // exit();
    ...
    public function stock($from_date = null, $to_date = null, $outlet_id = null, $product_id = null, $finished_raw = null)
    {
        ...
        $stockin = $this->db->select('sum(a.quantity) as totalSalesQnty')
            ...
            // ->where('b.date <=', $to_date)
            // ->where('b.outlet_id', $outlet_id)
            ->get()
            ->row();

        $warrenty_stock = $this->db->select('sum(ret_qty) as totalWarrentyQnty')
            ->from('warrenty_return')
            ->where('product_id', $product_id)
            ->get()
            ->row();
        ...
        $stockout_outlet = $this->db->select('sum(a_qty) as totaloutletQnty')
            ...
            ->where('rqsn_details.is_return', 0)
            ->where('rqsn_details.product_id', $product_id)
            // ->where('rqsn.date >=', $from_date)
            // ->where('rqsn.date <=', $to_date)
        ...
        $used_qty = $this->db->select('SUM(usage_qty) as used_qty')
            ...
            ->join('production', 'item_usage.production_id=production.pro_id', 'left')
            ->where('item_usage.item_id', $product_id)
            // ->where('production.date >=', $from_date)
            // ->where('production.date <=', $to_date)
            ->group_by('item_usage.item_id')
            ->get()
            ->row();
        ...
        $production_qty = $this->db->select('SUM(quantity) as pro_qty')
            ...
            ->where('production_goods.product_id', $product_id)
            // ->where('production.date >=', $from_date)
            // ->where('production.date <=', $to_date)
        ...
        $open_stock = $this->db->select('stock_qty')
            ->from('opening_inventory')
            ->where('opening_inventory.product_id', $product_id)
            // ->where('opening_inventory.date >=', $from_date)
            // ->where('opening_inventory.date <=', $to_date)
            ->get()
            ->row();
        ...
        $total_in = (!empty($open_stock->stock_qty) ? $open_stock->stock_qty : 0) + (!empty($stockout->totalPurchaseQnty) ? $stockout->totalPurchaseQnty : 0) + (!empty($production_qty->pro_qty) ? $production_qty->pro_qty : 0) -
            (!empty($return_given->totalReturnQnty) ? $return_given->totalReturnQnty : 0);
        ...
        if ($finished_raw == 1) {
            ...
            $transfer_item = $this->db->select('SUM(transfer_item_details.quantity) as transfer_item')
                ...
                ->join('transfer_items', 'transfer_item_details.pro_id=transfer_items.pro_id', 'left')
                ->where('transfer_item_details.product_id', $product_id)
                // ->where('transfer_items.date >=', $from_date)
                // ->where('transfer_items.date <=', $to_date)
                ->group_by('transfer_item_details.product_id')
                ->get()
                ->row();
        ...
        $phy_count = $this->db->select('SUM(a.difference) as phy_qty')
            ...
            // ->where('a.create_date >=', $from_date)
            // ->where('a.create_date <=', $to_date)
            // ->where('b.outlet_id', $outlet_id)
            ->group_by('a.product_id')
            ->order_by('a.id', 'desc')
            ->get()
        ...
        $stock = ($total_in - $total_out - $newStock - $stockout->damaged_qty) + $diff;

        // $stockout->damaged_qty
        return $stock;
    }

    public function expiry_outlet_stock_report($postData = null, $post_product_id = null, $from_date = null, $to_date = null)
    {
        ...
        if (!$post_product_id) {
            $this->db->order_by($columnName, $columnSortOrder);
            $this->db->group_by(array('p.purchase_id', 'b.product_id'));
            $this->db->limit($rowperpage, $start);
        }
        ...
        foreach ($records as $record) {
            ...
            $total_in = $stockout->totalPurchaseQnty + $total_purchase->total_purchase;
    ...
    public function expiry_outlet_stock_report_new($postData = null, $post_product_id = null, $from_date = null, $to_date = null)
    {
        ...
        if (!$post_product_id) {
            ...
            // $searchQuery = "";
            // if ($searchValue != '') {
            //     $searchQuery = " (b.product_name like '%" . $searchValue . "%' or b.product_model like '%" . $searchValue . "%'  or b.sku like '%" . $searchValue . "%') ";
            // }
            ...
            // $this->db->join('cats c', 'b.category_id=c.id', 'left');
            // if ($product_sku != '') {
            //     $this->db->where_in('b.sku', $product_sku);
            // }
            // if (isset($cat_id) && $cat_id != '') {
            //     $this->db->like('b.category_id', $cat_id, 'both');
            // }
    ...
    public function expiry_outlet_stock($post_product_id = null, $is_exp = null)
    {
        ...
        $total_wastage_qnty = $this->db->select('sum(wastage_dec.wastage_quantity) as total_wastage_qnty')
            ...
            ->row();
         // For Service Invoice
         $total_sold_qnty = $this->db->select('sum(service_invoice_details.quantity) as total_qnty')
         ...
         ->where('service_invoice_details.product_id', $post_product_id)
        ...
        $expired_stock -= (!empty($total_sold_qnty->total_qnty) ?  $total_sold_qnty->total_qnty : 0);
    ...
    {
        $this->load->model('warehouse');
        $this->db->select('a.*, b.*, sz.size_name,x.first_name,x.last_name, cl.color_name, d.product_id,d.product_name, d.product_model')
            ->from('rqsn a')
            ->join('rqsn_details b', 'a.rqsn_id = b.rqsn_id')
            ->join('product_information d', 'd.product_id=b.product_id')
            ->join('size_list sz', 'd.size=sz.size_id', 'left')

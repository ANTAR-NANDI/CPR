...
class Suppliers extends CI_Model
{
    ...
    public function getSupplierList($postData = null)
    {
        ...
        foreach ($records as $record) {
            ...
            $supplier_details = '  <a href="' . $base_url . 'Csupplier/supplier_due_payment/' . $record->supplier_id . '" class="" >' . $record->supplier_name .  '</a>';
    ...
    public function supplier_product_info($supplier_id, $product_id)
    {
        // print_r('je');
        // print_r($supplier_id);
        // exit();
    ...
    public function pr_supp_price($product_id)
    {

        $q = $this->db->select('supplier_price')
            ->from('supplier_product')
            ->where('product_id', $product_id)
            ->get();
    ...
    /////////////////////////////////////////////////////////Supplier Payment /////////////////////////////////////
     // Supplier due Payment View
    public function supplier_due_payment($supplier_id)
    {
        $this->db->select('product_purchase.*');
        $this->db->from('product_purchase');
        $this->db->where('product_purchase.due_amount >', 0);
        $this->db->where('product_purchase.supplier_id', $supplier_id);
        return $this->db->get()->result_array();
            
    }

    // Supplier due Payment Entry
    public function supplier_due_update_entry(){
        // echo "<pre>";
        // print_r($_POST);
        // exit();
        ...
        if($total_paid_amount != $total_pay_amount)
        {
            ...
            $Supplier_Headcode = $sup_coa->HeadCode;
            ...
            foreach($pay_amount as $key => $value)
            {
                if($value > 0)
                {
                    ...
                    $this->db->where('purchase_id', $purchase_id[$key]);
                    $this->db->update('product_purchase', $data);         
                 /////////////////////////////////////////////////////////////////////////////////////////////////////////
                    $supplier_debit = array(
                        'VNo'            =>  $purchase_id[$key],
                        'Vtype'          =>  'Supplier Payment',
                        'VDate'          =>  $receive_date,
                        'COAID'          =>  $Supplier_Headcode,
                        'Narration'      =>  'Supplier Debit for Purchase',
                        'Debit'          =>  $value,
                        'Credit'         => 0,
                        'IsPosted'       =>  1,
                        'CreateBy'       =>  $receive_by,
                   ...
                   $payment_credit = array(
                    ...
                    'VDate'          =>  $receive_date,
                    'COAID'          =>  $COAID,
                    'Narration'      =>  'Payment Received for Purchase',
                    'Credit'         => $value,
                    'Debit'          =>  0,
                    'IsPosted'       =>  1,
                    'CreateBy'       =>  $receive_by,
